...
static IRAM_ATTR bool gdma_on_trans_eof_callback(gdma_channel_handle_t dma_chan,
...
int MbiBusParallel16::get_transfer_count() {
  return trans_done_count;
}

// ------------------------------------------------------------------------------
void MbiBusParallel16::config(const config_t &cfg) {
  _cfg = cfg;
...
//https://github.com/adafruit/Adafruit_Protomatter/blob/master/src/arch/esp32-s3.h
esp_err_t MbiBusParallel16::setup_lcd_dma_periph(void) {
  esp_err_t ret;
...
esp_err_t MbiBusParallel16::release(void) {
  if (_i80_bus) {
...
bool MbiBusParallel16::allocate_dma_desc_memory(size_t len) {
...
esp_err_t MbiBusParallel16::dma_transfer_start() {
...
esp_err_t MbiBusParallel16::send_stuff_once(void *data, size_t size_in_bytes, bool is_greyscale_data) {
