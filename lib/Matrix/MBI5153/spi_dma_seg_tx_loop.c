...
static uint32_t spi_dma_tx_channel_id(void) {
  const spi_dma_ctx_t* dma_ctx = spi_bus_get_dma_ctx(SPI2_HOST);
  int channel_id = 0;
  if (dma_ctx && dma_ctx->tx_dma_chan) {
    gdma_get_channel_id(dma_ctx->tx_dma_chan, &channel_id);
  }
  return (uint32_t)channel_id;
}
...
esp_err_t spi_dma_transfer_loop_unpause(void) {
  ...
  gdma_ll_tx_start(&GDMA, tx_dma_chan);
