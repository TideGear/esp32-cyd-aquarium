...
void demoTask(void* parameter) {
  for (;;) {
    ...
    state->environment.temperature_fahrenheit.value =
        DEFAULT_TEMPERATURE_VALUE * 9.0 / 5.0 + 32.0;
    ...
    // Randomize History
    for (int i = 0; i < 24; i++) {
      state->environment.temperature.history_24h[i] = DEFAULT_TEMPERATURE_VALUE;
      state->environment.temperature_fahrenheit.history_24h[i] =
          DEFAULT_TEMPERATURE_VALUE * 9.0 / 5.0 + 32.0;
      state->environment.humidity.history_24h[i] = random(0, 100);
      state->environment.co2.history_24h[i] = DEFAULT_CO2_VALUE;
    }
...
void mqttTask(void* parameter) {
  ...
  mqttManager.begin("mqtt-broker.local", 1883, &stateManager);
